...
struct Parse : RedisModule::Command<Parse> {
	int Run(const RedisModule::Args &args) {
...
*/
struct HGetSet : RedisModule::Commmand<HGetSet> {
	int Run(const RedisModule::Args &args) {
		...
		if (RedisModule_KeyType(key) != REDISMODULE_KEYTYPE_HASH && RedisModule_KeyType(key) != REDISMODULE_KEYTYPE_EMPTY) {
...
int testParse(RedisModuleCtx *ctx) {
	RedisModuleCallReply *r = RedisModule_Call(ctx, "example.parse", "ccc", "SUM", "5", "2");
...
struct MyModule : RedisModule::Module {
	Parse parse;
	HGetSet hgetset;
